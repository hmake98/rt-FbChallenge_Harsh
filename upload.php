...
$client->setClientId('-- Enter your Client Id here --');
...
$album_name = $res_json['albums']['data'][$_GET['id']]['name'];
...
$folder->setTitle('facebook_'.$name.'_album');
...
$child_folder = new Google_DriveFile();
$child_folder->setTitle($album_name);
$child_folder->setMimeType($mime_type_folder);
$child_folder->setParents(array($parent));

$child_new_folder = $service->files->insert($child_folder);

$child_id = $child_new_folder['id'];

$child_parent = new Google_ParentReference();
$child_parent->setId($child_id);
...
$p = 0;
for($s = 0; $s < sizeof($all_data[$_GET['id']]); $s++){
    $file = new Google_DriveFile();
    $file->setTitle($album_name.'-'.$p++.'.jpg');
    $file->setMimeType('image/jpeg');
    $file->setParents(array($child_parent));
    ini_set('max_execution_time', 300);
    $service->files->insert(
        $file,
            array(
                'data' => file_get_contents($all_data[$_GET['id']][$s]),
                'mimeType' => 'image/jpeg'
            )
        );
}
